...
//
    ...
    private static void testMapConcurrently(final OurMap<Integer, String> map, final int nrOfThreads) {
        CyclicBarrier barrier = new CyclicBarrier(nrOfThreads + 1);
        AtomicInteger putSum = new AtomicInteger(), threadNumber = new AtomicInteger();
            ...
            threads[i] = new Thread(() ->
            {
                ...
                try {
                    ...
                    for (int j = 0; j < 10; j++) {
                        ...
                        if (replacedVal == null) {
                            putSum.addAndGet(putKey); //Nothing was there, add to the sum
    ...
    private static void testMapSeq(final OurMap<Integer, String> map, int buckets, int locks) {
        System.out.printf("%n%s%n", map.getClass());
        assert map.size() == 0;
        assert !map.containsKey(117);
        assert !map.containsKey(-2);
        assert map.get(117) == null;
        forceReallocateBuckets(map, buckets, locks); //<- Call to reallocate buckets
        ...
        forceReallocateBuckets(map, buckets, locks);
    }

    private static void forceReallocateBuckets(OurMap map, int buckets, int locks) {
    ...
    public static double Mark7(String msg, IntToDoubleFunction f) {
        ...
        System.out.println(String.format("%15.1f", mean));
...
class StripedWriteMap<K, V> implements OurMap<K, V> {
    ...
    public V put(K k, V v) {
        ...
        final Holder<V> oldValue = new Holder<V>();
        ItemNode<K, V>[] bs;
        int afterSize;
        synchronized (locks[stripe]) {
            ...
            final ItemNode<K, V> node = bs[hash],
                    newNode = ItemNode.delete(node, k, oldValue);
    ...
    public V putIfAbsent(K k, V v) {
        ...
        synchronized (locks[stripe]) {
            ...
            if (!ItemNode.search(buckets[index], k, oldValue)) {
